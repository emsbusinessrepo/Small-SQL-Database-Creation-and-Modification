...
ALTER TABLE competitor
    ADD CONSTRAINT check_gender CHECK ( comp_gender IN (
        'M',
        'F',
        'U'
    ) );
...
ALTER TABLE competitor
    ADD CONSTRAINT check_unistatus CHECK ( comp_unistatus IN (
        'Y',
        'N'
    ) );

ALTER TABLE competitor
    ADD CONSTRAINT check_ec_relationship CHECK ( comp_ec_relationship IN (
        'P',
        'T',
        'G',
        'F'
...
COMMENT ON COLUMN competitor.comp_ec_relationship IS
    ...
    (‘P’ for Parent, ‘G’ for Guardian, ‘T’ for Partner, or ‘F’ for Friend)';
...
ALTER TABLE team ADD CONSTRAINT team_un UNIQUE ( team_name,
                                                 carn_date );
...
ALTER TABLE team
    ADD CONSTRAINT team_entry_fk FOREIGN KEY ( event_id,
                                               entry_no )
